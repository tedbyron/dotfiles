(tset _G :tedbyron/pack [])
(tset _G :tedbyron/rock [])
...
(λ use-package! [identifier ?options]
  "Declares a plugin with its options. This macro adds it to the tedbyron/pack
  global table to later be used in the `unpack!` macro.
  See https://github.com/wbthomason/packer.nvim for information about the
  options."
  (assert-compile (str? identifier) "expected string for identifier" identifier)
  (if (not (nil? ?options)) (assert-compile (tbl? ?options) "expected table for options" ?options))
  (table.insert _G.tedbyron/pack (pack identifier ?options)))

(λ rock! [identifier ?options]
  "Declares a rock with its options. This macro adds it to the tedbyron/rock
  global table to later be used in the `unpack!` macro.
  See https://github.com/wbthomason/packer.nvim for information about the
  options."
  (assert-compile (str? identifier) "expected string for identifier" identifier)
  (if (not (nil? ?options)) (assert-compile (tbl? ?options) "expected table for options" ?options))
  (table.insert _G.tedbyron/rock (rock identifier ?options)))
...
(λ unpack! []
  ...
  (let [packs (icollect [_ v (ipairs _G.tedbyron/pack)] `(use ,v))
        rocks (icollect [_ v (ipairs _G.tedbyron/rock)] `(use_rocks ,v))
        use-sym (sym :use)]
    (tset _G :themis/pack [])
    (tset _G :themis/rock [])
